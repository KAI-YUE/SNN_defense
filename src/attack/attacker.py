...
def grad_inv(attacker, grad, x, onehot, model, config, logger):    
    ...
    if not config.snn_defense:
        ...
        # so if we use snn defense, we need to sample a conventional model first
        # now we simulate sampling a snn from a learned distribution
        sampled_snn = nn_registry[config.model](config)
        sampled_snn.load_state_dict(model.state_dict())
        sampled_snn = sampled_snn.to(config.device)
        ...
        # if config.fedalg != "fedavg":
        if "fedavg" not in config.fedalg:
            dummy_data = attacker.grad_inv(grad, x, onehot, sampled_snn, logger)
        else:
            dummy_data = attacker.multistep_attack(grad, x, onehot, sampled_snn, config.tau, logger)
